  ...
  stages{
    ...
    stage('Test') {
     steps {
       ...
       sh '''
       egoservice-api/test_stage/test.sh
       '''
    ...
    stage('Build') {
      steps {
        sh '''
        egoservice-api/build_stage/build.sh
        '''
      }
    }
    stage('Publish') {
      steps {
        sh '''
        egoservice-api/publish_stage/publish.sh
        '''
    ...
    stage('Deploy Dev') {
      ...
      steps {
        ...
        sh '''
        egoservice-api/deploy_stage/deploy.sh dev
        '''
    ...
    stage('Retag with pre-release'){
      ...
      steps {
        ...
        sh '''
          MANIFEST=$(aws ecr batch-get-image --repository-name kf-api-egoserverservice --image-ids imageTag=latest --query images[].imageManifest --output text)
          aws ecr put-image --repository-name kf-api-egoserverservice --image-tag "prerelease-$tag" --image-manifest "$MANIFEST"
        '''
      }
    }
    stage('Deploy QA') {
     ...
     steps {
       ...
       sh '''
       egoservice-api/deploy_stage/deploy.sh qa
       '''
       slackSend (color: '#41aa58', message: ":white_check_mark: DEPLOYED TO QA: (${env.BUILD_URL})")
     }
    }
    stage("Promotion egoservice-api to PRD") {
      when {
             expression {
               return env.BRANCH_NAME == 'master';
             }
      ...
      steps {
             script {
                     env.DEPLOY_TO_PRD = input message: 'User input required',
                                     ...
                                     parameters: [choice(name: 'egoservice-api: Deploy to PRD Environment', choices: 'no\nyes', description: 'Choose "yes" if you want to deploy the PRD server')]
             }
     }
    }
    stage('Retag with release'){
      ...
      steps {
        ...
        sh '''
          MANIFEST=$(aws ecr batch-get-image --repository-name kf-api-egoserverservice --image-ids imageTag="prerelease-$tag" --query images[].imageManifest --output text)
          aws ecr put-image --repository-name kf-api-egoserverservice --image-tag "$tag" --image-manifest "$MANIFEST"
        '''
      }
    }
    stage('Deploy PRD') {
     ...
     steps {
       ...
       sh '''
       egoservice-api/deploy_stage/deploy.sh prd
       '''
